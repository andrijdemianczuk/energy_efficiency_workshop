...
# MAGIC - Compare trials in the [MLflow experiment](#mlflow/experiments/2743357858718226).
...
input_data_path = mlflow.artifacts.download_artifacts(run_id="385c2f6718ff4fa998459799d0694b2d", artifact_path="data", dst_path=input_temp_dir)
...
# MAGIC `["incident_type"]` are dropped in the pipelines. See the Alerts tab of the AutoML Experiment page for details on why these columns are dropped.

# COMMAND ----------

from databricks.automl_runtime.sklearn.column_selector import ColumnSelector
supported_cols = ["incident_lsd", "volume_units", "incident_range", "volume_released", "source", "incident_meridian", "jurisdiction", "incident_section", "public_affected", "licence_type", "injury_count", "area_affected", "sensitive_area", "volume_recovered", "fatality_count", "strike_area", "incident_number", "cause_category", "incident_township", "cause_type", "environment_affected", "substance_released", "release_offsite", "wildlife_livestock_affected", "failure_type", "licensee_id"]
...
bool_transformers = [("boolean", bool_pipeline, ["release_offsite", "sensitive_area", "volume_units"])]
...
num_imputers = []
num_imputers.append(("impute_mean", SimpleImputer(), ["area_affected", "cause_category", "cause_type", "environment_affected", "failure_type", "fatality_count", "incident_lsd", "incident_meridian", "incident_number", "incident_range", "incident_section", "incident_township", "injury_count", "jurisdiction", "licence_type", "licensee_id", "public_affected", "release_offsite", "sensitive_area", "source", "strike_area", "substance_released", "volume_recovered", "volume_released", "volume_units", "wildlife_livestock_affected"]))
...
numerical_transformers = [("numerical", numerical_pipeline, ["incident_lsd", "volume_units", "incident_range", "volume_released", "source", "incident_meridian", "jurisdiction", "incident_section", "public_affected", "licence_type", "injury_count", "area_affected", "sensitive_area", "volume_recovered", "fatality_count", "strike_area", "incident_number", "cause_category", "incident_township", "cause_type", "environment_affected", "substance_released", "release_offsite", "wildlife_livestock_affected", "failure_type", "licensee_id"])]
...
categorical_one_hot_transformers = [("onehot", one_hot_pipeline, ["area_affected", "cause_category", "environment_affected", "fatality_count", "incident_meridian", "injury_count", "licence_type", "public_affected", "wildlife_livestock_affected"])]
...
# MAGIC - All the runs are logged under [this MLflow experiment](#mlflow/experiments/2743357858718226)
...
def objective(params):
  with mlflow.start_run(experiment_id="2743357858718226") as mlflow_run:
...
space = {
  "C": 0.17557554126131178,
  "penalty": "l1",
  "solver": "saga",
  "random_state": 62545389,
}
...
if shap_enabled:
    ...
    train_sample = X_train.sample(n=min(100, X_train.shape[0]), random_state=62545389)

    # Sample some rows from the validation set to explain. Increase the sample size for more thorough results.
    example = X_val.sample(n=min(100, X_val.shape[0]), random_state=62545389)
...
displayHTML(f"<a href=#mlflow/experiments/2743357858718226/runs/{ mlflow_run.info.run_id }/artifactPath/model> Link to model run page </a>")
